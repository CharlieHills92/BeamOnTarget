...
from gui.results_csv_loader import load_simulation_results
...
class ResultsTab(ttk.Frame):
    ...
    def _load_summary_csv(self):
        ...
        result = load_simulation_results(
            outdir_abs, selected, self.var_csv_use_smoothed.get())
        first_merged = result["first_merged"]
        first_display_order = result["first_order"]
        first_raw_path = result["first_raw_path"]
        n_loaded = result["n_loaded"]
        n_errors = result["n_errors"]

        self._csv_plot_data.clear()
        self._csv_plot_data.update(result["plot_data"])
